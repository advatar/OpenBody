...
The bundled simulation provider is intentionally deterministic and narrow. It only replays the exact bundled subject state, perturbation, scopes, and 7,200-second horizon; every mismatch produces a governed abstention. It exists to test OpenBody interoperability, not to act as a clinical model. The reference host does not implement an authorization scheme and advertises none. Production hosts must add external authority enforcement and qualified model adapters while preserving the same protocol semantics.
